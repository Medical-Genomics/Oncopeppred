...
def compute_pcp(sequence):
    """Compute 20 physicochemical properties (PCP) for a given sequence."""
    pcp_features = np.zeros(20)
    seq_length = len(sequence)
    
    if seq_length == 0:
        return pcp_features  # Return all zeros for empty sequences

    # Use the first 20 properties from PCP_GROUPS
    pcp_groups_list = list(PCP_GROUPS.items())[:20]  # Only first 20 properties

    # Iterate through each property and compute its frequency
    for idx, (pcp_name, residues) in enumerate(pcp_groups_list):
        count = sum(1 for aa in sequence if aa in residues)  # Count occurrences
        pcp_features[idx] = count / seq_length  # Convert to composition percentage
    
    return pcp_features

# Updated function to extract AAC (Amino Acid Composition)
def aac_new(sequence):
    amino_acids = 'ACDEFGHIKLMNPQRSTVWY'
    sequence = sequence.upper()
    aac_values = [sequence.count(aa) / len(sequence) for aa in amino_acids]
    return np.array(aac_values)

# Updated function for dipeptide composition (DPC)
def dpc_new(sequence, k=1, remove_zero_cols=False):
    dipeptides = [a + b for a in string.ascii_uppercase for b in string.ascii_uppercase]
    ...
    for i in range(len(sequence) - k):
        dipeptide = sequence[i] + sequence[i + k]
        if dipeptide in counts:
            counts[dipeptide] += 1

    length = len(sequence) - k
    dpc_features = np.array([counts[dp] / length if length > 0 else 0 for dp in dipeptides])

    # Ensure the feature vector is of length 400
    if len(dpc_features) != 400:
        dpc_features = np.zeros(400)  # Fill with zeros if it's not 400
...
# Function to preprocess peptide sequences for model input
def preprocess_sequence(seq, max_length=456):
    # Define amino acid to integer mapping (for example)
    ...
    seq = seq.upper()
    seq_int = [amino_acid_to_int[aa] for aa in seq if aa in amino_acid_to_int]

    # Pad sequence to the desired length
    if len(seq_int) < max_length:
        seq_int = seq_int + [0] * (max_length - len(seq_int))  # Padding with zeros
